...
def custom_formatter(root_path, meta_file, **kwargs):
    """Custom formatter for the given folder structure."""
    items = []

    # Debug print for root_path and meta_file
    print("Root path:", root_path)
    print("Meta file:", meta_file)

    # Iterate through each speaker directory
    for speaker_dir in os.listdir(root_path):
        print("Speaker directory:", speaker_dir)
        speaker_path = os.path.join(root_path, speaker_dir)
        if not os.path.isdir(speaker_path):
            continue

        # Locate the metadata file within the speaker directory
        meta_path = os.path.join(speaker_path, meta_file)
        print("Meta path:", meta_path)
        if not os.path.exists(meta_path):
            print(f"Metadata file '{meta_file}' not found in '{speaker_dir}'")
            continue

        # Extract speaker name from the speaker directory name
        speaker_name = speaker_dir

        # Read metadata file and process entries
        with open(meta_path, "r", encoding="utf-8") as meta_file_obj:  # Change variable name to avoid shadowing
            for line in meta_file_obj:
                cols = line.split("|")
                # print(cols)
                c1 = cols[0].strip()
                if(c1.endswith(".wav")):
                    ...
                    c1+=".wav"
                wav_file = os.path.join(speaker_path, "wavs", c1)
                try:
                    
                    text = cols[1]  # Assuming text is the second column in metadata.csv
                except :
                    print(c1)
